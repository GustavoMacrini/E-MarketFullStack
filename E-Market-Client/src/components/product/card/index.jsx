...
function ProductCard({ openModal}) {
    ...
    return (
        <div className={style.wrapper}>            
            ...
            <img className={style.image} src={image} onClick={() => openModal({ title, value, image, description })}></img>                           
            <h1 className={style.title} onClick={() => openModal({ title, value, image, description })}>{title}</h1>
